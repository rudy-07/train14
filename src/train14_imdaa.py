...
parser.add_argument('--imdaa_surface_dir', default='/path/to/imdaa_surface_dir', type=str, help='path to IMDAA surface data')
parser.add_argument('--imdaa_upper_dir', default='/path/to/imdaa_upper_dir/', type=str, help='path to IMDAA upper air data')
...
parser.add_argument('--output_dir', default='epochs_pangulite_era5', type=str, help='checkpoint output directory')
parser.add_argument('--log_dir', default='train_logs_pangulite_era5', type=str, help='training log directory')
...
parser.add_argument('--resume', action='store_true',
                    help='resume from pangu_lite_era5_latest.pth in --output_dir if it exists')
...
IMDAA_BLACKLIST_DATES = {
    "20001103","20001104","20001105","20001106","20001107","20001115","20001116","20001119",
    "20111014","20111017",
    "20140823","20140824","20140908","20140911","20140913","20140915","20140923","20140925","20140926",
    "20141007","20141008","20141009","20141012","20141015","20141016","20141021","20141023","20141024","20141027",
    "20141127","20141213","20141219","20141225","20141226","20141228","20141230","20141231",
    "20150327","20150329","20150330","20150401","20150402","20150412","20150413","20150414","20150415",
    "20150419","20150421","20150422","20150424","20150425","20150427","20150429","20150430",
    "20150501","20150502","20150503","20150504","20150506","20150509","20150511","20150512","20150531",
    "20150601","20150602","20150603","20150604","20150605","20150606","20150607","20150609","20150610",
    "20150611","20150612","20150613","20150614","20150615","20150616","20150617","20150619","20150620",
    "20150621","20150622","20150623","20150625","20150629",
    "20150701","20150703","20150717","20150723","20150731","20150801",
    "20151009","20151011","20151013","20151014","20151015","20151017",
    "20160516",
    "20200708","20200709","20200713","20200715","20200717","20200718",
    "20200726","20200727","20200729","20200730","20200731",
    "20200802","20200803"
}
...
def surface_transform(mean_path, std_path):
    ...
    mean_seq = [float(np.nan_to_num(surface_mean_npy[i], nan=0.0)) for i in range(len(SURFACE_VARIABLES))]
    std_seq = [max(1e-6, float(np.nan_to_num(surface_std_npy[i], nan=1.0))) for i in range(len(SURFACE_VARIABLES))]
...
def upper_air_transform(mean_path, std_path):
    ...
    for pl in pLevels:
        ...
        mean_seq = [float(np.nan_to_num(upper_mean_npy[pl_idx, 0, 0, i], nan=0.0)) for i in range(len(UPPER_VARIABLES))]
        std_seq = [max(1e-6, float(np.nan_to_num(upper_std_npy[pl_idx, 0, 0, i], nan=1.0))) for i in range(len(UPPER_VARIABLES))]
...
class IMDAADatasetFromFolder(Dataset):
    def __init__(
        self,
        surface_dir,
        upper_dir,
        ...
        self.aux_data_dir = aux_data_dir or "./aux_data"
    ...
    def get_lat_lon(self):
        ...
        ts = self.samples[0][0]
        date_str = pd.to_datetime(ts).strftime("%Y%m%d%H")
        date_folder = pd.to_datetime(ts).strftime("%Y%m%d")
        var = self.surface_variables[0]
        surf_var_map = {
            'msl': ('PRMSL-msl', f'PRMSL-msl_{date_str}_ncum_imdaa_reanl_2df.grb2'),
            'u10': ('UGRD-10m', f'UGRD-10m_{date_str}_ncum_imdaa_reanl_2df.grb2'),
            'v10': ('VGRD-10m', f'VGRD-10m_{date_str}_ncum_imdaa_reanl_2df.grb2'),
            't2m': ('TMP-2m', f'TMP-2m_{date_str}_ncum_imdaa_reanl_2df.grb2'),
        }
        folder, fname = surf_var_map[var]
        path = join(self.surface_dir, date_folder, folder, fname)
        import os
    ...
    def __getitem__(self, index):
        while True:
            try:
                ...
                import pandas as pd
                print(f"[WARNING] Skipping corrupted sample at index {index} (Input: {pd.to_datetime(input_time)}, Target: {pd.to_datetime(target_time)}): {e}")
    ...
    def _build_samples(self):
        ...
        # Scan surface dir for YYYYMMDD
        for folder in os.listdir(self.surface_dir):
            if len(folder) == 8 and folder.isdigit():
                # Inside check for any .grb2 in TMP-2m
                tmp_dir = join(self.surface_dir, folder, 'TMP-2m')
                if os.path.exists(tmp_dir):
                    for file in os.listdir(tmp_dir):
                        if file.endswith('.grb2'):
                            # Extract YYYYMMDDHH
                            # e.g. TMP-2m_2009011000_ncum_imdaa_reanl_2df.grb2
                            parts = file.split('_')
                            if len(parts) >= 2 and parts[1].isdigit():
                                date_str = parts[1]
                                if date_str[:8] in IMDAA_BLACKLIST_DATES:
                                    continue
                                ts = np.datetime64(f"{date_str[:4]}-{date_str[4:6]}-{date_str[6:8]}T{date_str[8:10]}:00:00", "s")
                                available_dates.append(ts)
                                
        available_dates = sorted(list(set(available_dates)))
        valid_times = []
        for ts in available_dates:
            if self.range_start <= ts <= self.range_end and self._matches_stride(ts):
                valid_times.append(ts)
                
        # Validate pairs
        samples = []
        valid_keys = set([int(ts.astype(np.int64)) for ts in available_dates])
        for ts in valid_times:
            target_ts = (ts + lead).astype('datetime64[s]')
            if self.range_start <= target_ts <= self.range_end and int(target_ts.astype(np.int64)) in valid_keys:
                samples.append((ts.astype('datetime64[ns]'), target_ts.astype('datetime64[ns]')))
        return samples

    def _to_2d_array(self, value, var_name):
        arr = np.asarray(value, dtype=np.float32)
        arr = np.squeeze(arr)
        if arr.ndim != 2:
            pass
        if hasattr(self, 'lat') and self.lat is not None and self.lat[0] < self.lat[-1]:
            arr = arr[::-1, :].copy()
        return arr
    ...
    def _get_data(self, date):
        date_str = pd.to_datetime(date).strftime("%Y%m%d%H")
        date_folder = pd.to_datetime(date).strftime("%Y%m%d")
        ...
        for var in self.surface_variables:
            ...
            path = join(self.surface_dir, date_folder, folder, fname)
            ds = xr.open_dataset(path, engine='cfgrib', backend_kwargs={'indexpath': ''})
            val = ds[list(ds.data_vars)[0]].values
            surface_data_list.append(self._to_2d_array(val, var))
            ds.close()
        ...
        surface_data = torch.nan_to_num(surface_data, nan=0.0, posinf=0.0, neginf=0.0)
        ...
        for pl in self.upper_air_pLevels:
            ...
            for var in self.upper_air_variables:
                if var == 'q':
                    folder_rh = up_var_map['rh']
                    fname_rh = f"RH-{pl}mb_{date_str}_ncum_imdaa_reanl_prl.grb2"
                    ds_rh = xr.open_dataset(join(self.upper_dir, date_folder, folder_rh, fname_rh), engine='cfgrib', backend_kwargs={'indexpath': ''})
                    rh_val = self._to_2d_array(ds_rh[list(ds_rh.data_vars)[0]].values, 'rh')
                    ds_rh.close()

                    folder_t = up_var_map['t']
                    fname_t = f"TMP-{pl}mb_{date_str}_ncum_imdaa_reanl_prl.grb2"
                    ds_t = xr.open_dataset(join(self.upper_dir, date_folder, folder_t, fname_t), engine='cfgrib', backend_kwargs={'indexpath': ''})
                    t_val = self._to_2d_array(ds_t[list(ds_t.data_vars)[0]].values, 't')
                    ds_t.close()

                    q_val = self._relative_humidity_to_specific_humidity(rh_val, t_val, pl)
                    var_data_list.append(q_val)
                else:
                    folder = up_var_map[var]
                    if var == 't' and t_val is not None:
                        val = t_val
                    else:
                        prefix = folder.split('-')[0]
                        fname = f"{prefix}-{pl}mb_{date_str}_ncum_imdaa_reanl_prl.grb2"
                        ds = xr.open_dataset(join(self.upper_dir, date_folder, folder, fname), engine='cfgrib', backend_kwargs={'indexpath': ''})
                        val = self._to_2d_array(ds[list(ds.data_vars)[0]].values, var)
                        if var == 't':
                            t_val = val
                        ds.close()
                    var_data_list.append(val)
            pl_data = np.stack(var_data_list, axis=0)
            pl_data_list.append(
                self.upper_air_transform[pl](torch.from_numpy(pl_data.astype(np.float32)))
            )
        
        upper_air_data = torch.stack(pl_data_list, dim=1) # [Vars, Pl, Lat, Lon]
        upper_air_data = torch.nan_to_num(upper_air_data, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    def _load_constant_mask(self):
        ...
        if mask.ndim == 4:
            mask = mask[0]
        land_mask = torch.nan_to_num(torch.from_numpy(mask[0]), nan=0.0)
        soil_type = torch.nan_to_num(torch.from_numpy(mask[1]), nan=0.0)
        topography = torch.nan_to_num(torch.from_numpy(mask[2]), nan=0.0)
    ...
    def _load_const_h(self):
        ...
        return torch.nan_to_num(torch.from_numpy(const_h), nan=0.0).unsqueeze(0).unsqueeze(0)
...
class Pangu_lite(nn.Module):
    def __init__(self, img_size=(721, 1440), embed_dim=192, num_heads=(6, 12, 12, 6), window_size=(2, 6, 12), residual=True):
        super().__init__()
        self.residual = residual
        import math
        H, W = img_size
        Lat_enc = math.ceil(H / 4)
        Lon_enc = math.ceil(W / 4)
        Lat_bot = math.ceil(Lat_enc / 2)
        Lon_bot = math.ceil(Lon_enc / 2)
        ...
        self.layer1 = BasicLayer(
            dim=embed_dim,
            input_resolution=(8, Lat_enc, Lon_enc),
            depth=2,
            num_heads=num_heads[0],
            window_size=window_size,
            drop_path=drop_path[:2]
            )
        self.downsample = DownSample(in_dim=embed_dim, input_resolution=(8, Lat_enc, Lon_enc), output_resolution=(8, Lat_bot, Lon_bot))
        self.layer2 = BasicLayer(
            dim=embed_dim * 2,
            input_resolution=(8, Lat_bot, Lon_bot),
            depth=6,
        ...
        self.upsample = UpSample(embed_dim * 2, embed_dim, (8, Lat_bot, Lon_bot), (8, Lat_enc, Lon_enc))
        ...
        self.patchrecovery2d = PatchRecovery2D(img_size, (4, 4), 2 * embed_dim, 4)
        self.patchrecovery3d = PatchRecovery3D((13, H, W), (2, 4, 4), 2 * embed_dim, 5)
        if self.residual:
    ...
    def forward(self, surface, surface_mask, upper_air, const_h):
        ...
        if const_h.size(0) == 1 or const_h.size(2) == 1:
            const_h = const_h.expand(upper_air.size(0), -1, upper_air.size(2), -1, -1)
...
if __name__ == "__main__":
    ...
    aux_data_dir = opt.aux_data_dir or "./aux_data"
    ...
    if rank == 0:
        train_set = IMDAADatasetFromFolder(
            opt.imdaa_surface_dir,
            opt.imdaa_upper_dir,
            "train",
        ...
        val_set = IMDAADatasetFromFolder(
            opt.imdaa_surface_dir,
            opt.imdaa_upper_dir,
            "valid",
            lead_hours=opt.horizon_hours,
    ...
    if rank == 0:
        print(f"IMDAA Surface dir: {opt.imdaa_surface_dir}")
        print(f"IMDAA Upper dir:   {opt.imdaa_upper_dir}")
    ...
    lat_arr, lon_arr = getattr(train_set, 'lat', None), getattr(train_set, 'lon', None)
    if lat_arr is not None and lon_arr is not None:
        img_size = (len(lat_arr), len(lon_arr))
        print(f"Dynamically inferred IMDAA grid size: {img_size}")
    else:
        img_size = (721, 1440)
        print("Fallback to 721x1440 grid size.")
    pangu_lite = Pangu_lite(img_size=img_size, residual=opt.residual)
    ...
    if rank == 0:
        ...
        print(f"PanguLite ERA5 parameters: {n_params:,}", flush=True)
    ...
    if opt.resume and rank == 0:
        latest_path = os.path.join(opt.output_dir, "pangu_lite_era5_latest.pth")
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        if rank == 0:
            ...
            checkpoint = {
                ...
                "model_version": "pangulite_era5_monthly_r_to_q_pangu_order_2x4x4_consth",
            ...
            if opt.save_every > 0 and epoch % opt.save_every == 0:
                torch.save(checkpoint, os.path.join(opt.output_dir, f"pangu_lite_era5_epoch_{epoch:03d}.pth"))
            if should_validate and val_score == best_val and patience_counter == 0:
                # val_score just improved (patience reset to 0 above)
                torch.save(checkpoint, os.path.join(opt.output_dir, "pangu_lite_era5_best.pth"))
        ...
        if rank == 0:
            ...
            data_frame.to_csv(os.path.join(save_root, "pangulite_era5_logs.csv"), index=False)
