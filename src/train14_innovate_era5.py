...
#
# Standard Conv-based patch embedding variant of train14_small_era5.py  (v1-fixed).
#
# Architecture:
#   PatchEmbed2D  → Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
#   PatchEmbed3D  → Conv3d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
#
# Note: The earlier "CrossAttn" patch-embedding version produced horizontal banding
#   artifacts due to the shared-query design (position-invariant attention with no
#   cross-patch communication). This version uses the proven Conv-based embedding
#   identical to train14_small_era5.py.
#
...
# (Cross-attention embed hyper-parameters removed — now using standard Conv2d/3D patch embedding)
...
class Pangu_lite(nn.Module):
    """
    Pangu-Lite with standard Conv2d/Conv3d patch embedding (v1-fixed).

    All architectural parameters (window sizes, depths, dims) are identical to
    train14_small_era5.py.  The earlier CrossAttn embedders are replaced with
    standard strided-Conv embedders to eliminate horizontal banding artifacts.
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        print(f"PanguLite-Innovate ERA5 (v1-fixed, Conv embed) parameters: {n_params:,}", flush=True)
